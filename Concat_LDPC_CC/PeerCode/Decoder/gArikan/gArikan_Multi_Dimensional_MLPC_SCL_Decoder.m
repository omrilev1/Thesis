function [Estimated_U,Estimated_L] = gArikan_Multi_Dimensional_MLPC_SCL_Decoder(L,GA_CRC_Length,m,m_SCL,Constellation_Mapping_Array,Y,U,X,Sigma,future_matrix,Is_Frozen_Bit_Index_Vec,Is_Genie_Aided)
    
    Constellation_2D_Dimension = size(Constellation_Mapping_Array,1);
 
    N = length(Y)./Constellation_2D_Dimension;
    ...
    [LL0,LL1] = AWGN_Multi_Dimensional_MLPC_LL(m,Constellation_Mapping_Array,Y,Sigma,future_matrix,1,Estimated_X_List);
    ...
    for i=1:1:m_SCL-1
        ...
        if(2^i>L) %לממש אלגוריתם שבודק איזה X היו ולא סתם להריץ הכל
            ...
            for j=1:1:L
                ...
                [LL0(j,:),LL1(j,:)] = AWGN_Multi_Dimensional_MLPC_LL(m,Constellation_Mapping_Array,Y,Sigma,future_matrix,i+1,Estimated_X_List(j,:,:));
            ...
            for j=1:1:size(past_matrix,2)
                ...
                [LL0_temp(j,:),LL1_temp(j,:)] = AWGN_Multi_Dimensional_MLPC_LL(m,Constellation_Mapping_Array,Y,Sigma,future_matrix,i+1,Estimated_X_temp);
    ...
    if(and(GA_CRC_Length>=0,is_correct))
        ...
        if(m_SCL==m) %חישוב מה המסלול הכי סביר
            ...
            for i=1:1:size(past_matrix,2)
                ...
                [LL0_temp(i,:),LL1_temp(i,:)] = AWGN_Multi_Dimensional_MLPC_LL(m,Constellation_Mapping_Array,Y,Sigma,future_matrix,m_SCL+1,Estimated_X_temp);
    ...
    for i=m_SCL+1:1:m
        ...
        lambda = AWGN_MLPC_Multi_Dimensional_LLR(m,Constellation_Mapping_Array,Y,Sigma,future_matrix,i,Estimated_X);
