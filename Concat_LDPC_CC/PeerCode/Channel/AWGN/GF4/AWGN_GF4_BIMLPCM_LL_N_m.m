 function [ll_p0,ll_p1,ll_p2,ll_p3] = AWGN_GF4_BIMLPCM_LL_N_m(m,BIPCM_m,Constellation_Mapping_Array,Y,Sigma,future_matrix,k,Estimated_X,g0)
    ...
    if(k>BIPCM_m)
        ...
        ll_p0 = log(sum(exp(-((temp_real-real(X0)).^2+(temp_imag-imag(X0)).^2)*exp_multiplier),1)).';
        ll_p1 = log(sum(exp(-((temp_real-real(X1)).^2+(temp_imag-imag(X1)).^2)*exp_multiplier),1)).';
        ll_p2 = log(sum(exp(-((temp_real-real(X2)).^2+(temp_imag-imag(X2)).^2)*exp_multiplier),1)).';
        ll_p3 = log(sum(exp(-((temp_real-real(X3)).^2+(temp_imag-imag(X3)).^2)*exp_multiplier),1)).';
        ...
        for k=1:2:m
            ...
            p0 = sum(exp(-((temp_real-real(X0)).^2+(temp_imag-imag(X0)).^2)*exp_multiplier),2);
            p1 = sum(exp(-((temp_real-real(X1)).^2+(temp_imag-imag(X1)).^2)*exp_multiplier),2);
            p2 = sum(exp(-((temp_real-real(X2)).^2+(temp_imag-imag(X2)).^2)*exp_multiplier),2);
            p3 = sum(exp(-((temp_real-real(X3)).^2+(temp_imag-imag(X3)).^2)*exp_multiplier),2);
