...
Constellation_Mapping_Array = Constellation_Mapping_Array - sign(Constellation_Mapping_Array).*(min(abs(Constellation_Mapping_Array))*(1/2));
Constellation_Mapping_Array = Constellation_Mapping_Array./sqrt(sum(abs(Constellation_Mapping_Array).^2)/(2^m));

[BLER(2,:),BER(2,:)] = Find_Frozen_Bits_And_Simulate(NumOfWorkers,SNR_Type,'MLPC',1,1,0,Constellation_Type,m,zeros(0),zeros(0),Constellation_Mapping_Array,N,R_Code_Bits,SNR_Vec,BLER_Max,Min_Num_Of_Errors_Sufficient_Statistics,file_path,plot_path,is_visible);
save(fullfile(file_path,file_name),'N','R_Code_Bits','Constellation_Type','m','SNR_Vec_dB_EbN0','SNR_Vec_dB_EsN0','BLER','BER'); 

%%
%----------------------------plot BLER----------------------------%

legend_name = {sprintf('MLPC %d%s SP',2^m,Constellation_Type),sprintf('MLPC %d%s SP',2^m,Constellation_Type)};
