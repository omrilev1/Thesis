...
for i=1:length(snr)
    ...
    for n=1:Niters
        ...
        tent_idx = find(mod(round((beta_kochman*x - TxSig)/(2*delta_kochman)),2) == 1);
        ...
        corrTuncel(n) = x_hat_Tuncel*ErrTuncel(n);
        ...
        %% Tent 
        x_hat_tent = FullMMSE_decoder_tent(RxSig_tent,y,beta_kochman,delta_kochman,rho,sigmaW);
...
plot(snr , SDR_Tent,'--','LineWidth',2,'MarkerSize',8,'MarkerEdgeColor','b','MarkerFaceColor',[0.5,0.5,0.5])
...
plot(snr , 10*log10((1 + snrLin)/(1 - rho^2)),'LineWidth',2)
plot(snr , SDR_Tuncel_FullMMSE,'-kd','LineWidth',2)
plot(snr , SDR_TuncelImproved_FullMMSE,'b-*','LineWidth',2)
plot(snr , SDR_Kochman,'--gs','LineWidth',2,'MarkerSize',8,'MarkerEdgeColor','b','MarkerFaceColor',[0.5,0.5,0.5])
plot(snr , 10*log10((1 + (1-rho^2)*snrLin)/(1-rho^2)),'r--','LineWidth',2)
...
xlabel('SNR [dB]'); ylabel('SDR [dB]');
legend('SDR OPTA - SI case : (1 + SNR)/(1-\rho^2)',...
    strcat('Tuncel [17] : \Delta = ',num2str(delta),' \alpha = ',num2str(alpha),' \beta = ',num2str(beta)),...
    strcat('Tuncel [17] with non-uniform weighting : \Delta = ',num2str(delta),' \alpha = ',num2str(alpha_tilde),' \beta = ',num2str(beta_tilde)),...
    strcat('Kochman Zamir [10] : \Delta = ',num2str(delta_kochman),' \gamma = ',num2str(beta_kochman)),'Linear Scheme  : (1 + (1-\rho^2)\cdotSNR)/(1-\rho^2)');
title(strcat('Distortion Vs SNR, \rho = ',num2str(rho)));
ylim([10 20]);
